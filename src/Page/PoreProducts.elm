module Page.PoreProducts exposing (Model, Msg, init, update, view, viewHeader)
...
import Pore exposing (Pore, PoreId, poreProductsDecoder)
import RemoteData exposing (WebData)


type alias Model = 
    { pore : WebData (List Pore)
    }


type Msg
    = FetchProducts
    | PoreProductsReceived (WebData (List Pore))


init : ( Model, Cmd Msg )
init = 
    ( { pore = RemoteData.Loading }, fetchProducts )

fetchProducts : Cmd Msg
fetchProducts = 
    Http.get 
        { url = "http://localhost:5019/pore/"
        , expect = 
            poreProductsDecoder
                |> Http.expectJson (RemoteData.fromResult >> PoreProductsReceived)
        }   
...
update msg model = 
    case msg of
        FetchProducts -> 
            ( { model | pore = RemoteData.Loading }, fetchProducts )

        PoreProductsReceived response -> 
            ( { model | pore = response }, Cmd.none ) 
...
viewPoreProducts : WebData (List Pore) -> Html Msg 
viewPoreProducts products =
    case products of
        RemoteData.NotAsked -> 
            text ""
            ...
            div [class "max-w-screen max-h-max"]
                [ section [class "grid grid-cols-7 grid-rows-1"] 
                    ...
                    , section [class "z-10 grid col-span-6 grid-flow-row auto-rows-max grid-cols-3 gap-1"]
                        (List.map viewPore actualProducts)  
                    ]
                ]
...
viewPore : Pore -> Html Msg
viewPore pore = 
    let
        productPath =
            "/pore/" ++ Pore.idToString pore.id
    in
    div [class "bg-slate-0 p-10 pb-10 mb-3 ml-3 mt-8 mr-14 h-6/7 w-7/8 rounded-md drop-shadow-md hover:drop-shadow-xl" ] 
        ...
        , article [class "flex justify-center item-center"]
            [ img [ src pore.imgSrc, class "w-3/4 h-4/5 mx-7 my-7"] 
                []
            ]
