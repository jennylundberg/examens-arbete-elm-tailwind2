module Page.ListProducts exposing (Model, Msg, init, update, view, viewHeader)
...
type Msg
    = FetchProducts
    | ProductsReceived (WebData (List Product))


init : ( Model, Cmd Msg )
init = 
    ( { products = RemoteData.Loading }, fetchProducts )
...
fetchProducts = 
    Http.get 
        ...
        , expect = 
            productsDecoder
                |> Http.expectJson (RemoteData.fromResult >> ProductsReceived)
        }   
            ...
            div [class "max-w-screen max-h-max"]
                [ section [class "grid grid-cols-7 grid-rows-1"] 
                    ...
                    , section [class "z-10 grid col-span-6 grid-row-3 grid-cols-3 gap-1 "]
                        (List.map viewProduct actualProducts)  
                    ]
                ]
...
viewProduct product = 
    let
        productPath =
            "/products/" ++ Product.idToString product.id
    in
    div [class "bg-slate-0 p-10 pb-10 mb-3 ml-3 mt-8 mr-14 h-6/7 w-7/8 rounded-md drop-shadow-md hover:drop-shadow-xl" ] 
        ...
        , article [class "flex justify-center item-center"]
            [ img [ src product.imgSrc, class "w-3/4 h-4/5 mx-7 my-7"] 
                []
            ]
