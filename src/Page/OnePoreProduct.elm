module Page.OnePoreProduct exposing (Model, Msg, init, update, view)
...
import Http
import Pore exposing (Pore, PoreId, poreDecoder, poreEncoder)
...
init : PoreId -> Nav.Key -> ( Model, Cmd Msg )
init poreId navKey =
    ( initialModel navKey, fetchProduct poreId )


initialModel : Nav.Key -> Model
initialModel navKey =
    { navKey = navKey
    , pore = RemoteData.Loading
    , saveError = Nothing
    }


fetchProduct : PoreId -> Cmd Msg
fetchProduct poreId =
    Http.get
        { url = "http://localhost:5019/pore/" ++ Pore.idToString poreId
        , expect =
            poreDecoder
...
type Msg = ProductReceived (WebData Pore)
    

update : Msg -> Model -> ( Model, Cmd Msg )
update msg model =
    case msg of
        ProductReceived pore ->
            ( { model | pore = pore }, Cmd.none )
...
viewAcneProduct : WebData Pore -> Html Msg
viewAcneProduct pore =
    case pore of
        RemoteData.NotAsked ->
            text ""
        ...
        RemoteData.Success productData ->
            viewPoreProductCard productData

        RemoteData.Failure httpError ->
            viewFetchError (buildErrorMessage httpError)


viewPoreProductCard : Pore -> Html Msg
viewPoreProductCard pore =
        ...
        [ section [class "flex item-center justify-center bg-slate-0 p-5 pb-10 m-3 mt-16 h-3/4 w-2/4 rounded-md drop-shadow-md hover:drop-shadow-xl mx-16"]
            [ section [ class "m-7" ]  
                ...
                , article [class "flex justify-center item-center"]
                    [ img [ src pore.imgSrc, class "max-w-xs max-h-80 w-3/4 h-4/5 mx-7 my-7"] 
                        []
                    ]
                ]
            , section [ class "m-12"]
