module Page.MouisturizingProducts exposing (Model, Msg, init, update, view, viewHeader)
...
import Mouisturizing exposing (Mouisturizing, MouisturizingId, mouisturizingProductsDecoder)
import RemoteData exposing (WebData)


type alias Model = 
    { mouisturizing : WebData (List Mouisturizing)
    }


type Msg
    = FetchProducts
    | MouisturizingProductsReceived (WebData (List Mouisturizing))


init : ( Model, Cmd Msg )
init = 
    ( { mouisturizing = RemoteData.Loading }, fetchProducts )

fetchProducts : Cmd Msg
fetchProducts = 
    Http.get 
        { url = "http://localhost:5019/mouisturizing/"
        , expect = 
            mouisturizingProductsDecoder
                |> Http.expectJson (RemoteData.fromResult >> MouisturizingProductsReceived)
        }   
...
update msg model = 
    case msg of
        FetchProducts -> 
            ( { model | mouisturizing = RemoteData.Loading }, fetchProducts )

        MouisturizingProductsReceived response -> 
            ( { model | mouisturizing = response }, Cmd.none ) 
...
view model = 
    div [class "bg-zinc-50"]
        [ viewHeader 
        , viewMouisturizingProducts model.mouisturizing
        ]
...
viewMouisturizingProducts : WebData (List Mouisturizing) -> Html Msg 
viewMouisturizingProducts products =
    case products of
        RemoteData.NotAsked -> 
            text ""
                ...
                [ section [class "grid grid-cols-7 grid-rows-1"] 
                    [ section [class "grid col-span-1 grid-cols-1 grid-rows-6"]
                        [ ul [class ""]
                            ...
                            , li [class "mt-4"]
                                [ a [ href poreProductPath, class "text-center text-sm m-1 mt-4 mb-4 ml-12 px-3 py-2 text-indigo-700 transition-colors duration-150 border border-indigo-500 rounded-lg focus:shadow-outline hover:bg-indigo-500 hover:text-slate-0"] 
                                    [ text "Pore" ]
                                ]
                            , li [class "mt-4"]
                    ...
                    , section [class "z-10 grid col-span-6 grid-row-3 grid-cols-3 gap-1 "]
                        (List.map viewMouisturizing actualProducts)  
                    ]
                ]
...
viewMouisturizing : Mouisturizing -> Html Msg
viewMouisturizing mouisturizing = 
    let
        productPath =
            "/mouisturizing/" ++ Mouisturizing.idToString mouisturizing.id
    in
    div [class "bg-slate-0 p-10 pb-10 mb-3 ml-3 mt-8 mr-14 h-6/7 w-7/8 rounded-md drop-shadow-md hover:drop-shadow-xl" ] 
        ...
        , article [class "flex justify-center item-center"]
            [ img [ src mouisturizing.imgSrc, class "w-3/4 h-4/5 mx-7 my-7"] 
                []
            ]
