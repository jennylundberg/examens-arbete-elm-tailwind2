module Page.AcneProducts exposing (Model, Msg, init, update, view, viewHeader)
...
import Acne exposing (Acne, AcneId, acneProductsDecoder)
import RemoteData exposing (WebData)


type alias Model = 
    { acne : WebData (List Acne)
    }


type Msg
    = FetchProducts
    | AcneProductsReceived (WebData (List Acne))


init : ( Model, Cmd Msg )
init = 
    ( { acne = RemoteData.Loading }, fetchProducts )

fetchProducts : Cmd Msg
fetchProducts = 
    Http.get 
        { url = "http://localhost:5019/acne/"
        , expect = 
            acneProductsDecoder
                |> Http.expectJson (RemoteData.fromResult >> AcneProductsReceived)
        }   
...
update msg model = 
    case msg of
        FetchProducts -> 
            ( { model | acne = RemoteData.Loading }, fetchProducts )

        AcneProductsReceived response -> 
            ( { model | acne = response }, Cmd.none ) 
...
viewAcneProducts : WebData (List Acne) -> Html Msg 
viewAcneProducts products =
    case products of
        RemoteData.NotAsked -> 
            text ""
            ...
            div [class "max-w-screen max-h-max"]
                [ section [class "grid grid-cols-7 grid-rows-1"] 
                    ...
                    , section [class "z-10 grid col-span-6 grid-flow-row auto-rows-max grid-cols-3 gap-1"]
                        (List.map viewAcne actualProducts)  
                    ]
                ]
...
viewAcne : Acne -> Html Msg
viewAcne acne = 
    let
        productPath =
            "/acne/" ++ Acne.idToString acne.id
    in
    div [class "bg-slate-0 p-10 pb-10 mb-3 ml-3 mt-8 mr-14 h-6/7 w-7/8 rounded-md drop-shadow-md hover:drop-shadow-xl" ] 
        ...
        , article [class "flex justify-center item-center"]
            [ img [ src acne.imgSrc, class "w-3/4 h-4/5 mx-7 my-7"] 
                []
            ]
