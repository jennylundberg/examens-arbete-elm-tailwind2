module Page.OneProduct exposing (Model, Msg, init, update, view)
...
import Http
import Product exposing (Product, ProductId, productDecoder, productEncoder)
...
init : ProductId -> Nav.Key -> ( Model, Cmd Msg )
init productId navKey =
    ( initialModel navKey, fetchProduct productId )
...
fetchProduct productId =
    Http.get
        { url = "http://localhost:5019/products/" ++ Product.idToString productId
        , expect =
            productDecoder
...
type Msg = ProductReceived (WebData Product)
...
viewProduct : WebData Product -> Html Msg
viewProduct product =
    case product of
        RemoteData.NotAsked ->
            text ""
        ...
        [ section [class "flex item-center justify-center bg-slate-0 p-5 pb-10 m-3 mt-16 h-3/4 w-2/4 rounded-md drop-shadow-md hover:drop-shadow-xl mx-16"]
            [ section [ class "m-7" ]  
                ...
                , article [class "flex justify-center item-center"]
                    [ img [ src product.imgSrc, class "max-w-xs max-h-80 w-3/4 h-4/5 mx-7 my-7"] 
                        []
                    ]
                ]
            , section [ class "m-12"]
